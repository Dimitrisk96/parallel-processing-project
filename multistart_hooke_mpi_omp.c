...
int main(int argc, char *argv[])
{
	...
	double local_best_fx = 1e10, local_best_pt[MAXVARS];
	
	int local_best_trial = -1, local_best_jj = -1;

	int my_rank, p, num, start,end ; 
	
	
	int tag1=40, tag2=60, tag3= 80, tag4 = 100;
	...
	{
	MPI_Send(&local_best_pt[0],nvars, MPI_DOUBLE,0, tag1, MPI_COMM_WORLD);
	
	MPI_Send(&local_best_fx,1, MPI_DOUBLE, 0, tag2, MPI_COMM_WORLD);
	
       MPI_Send(&local_best_trial, 1, MPI_INT, 0, tag3, MPI_COMM_WORLD);
	 
	MPI_Send(&local_best_jj, 1, MPI_INT, 0, tag4, MPI_COMM_WORLD);
	...
	
	{
	  ...
	  for(diergasia=1;diergasia<p;diergasia++)
	  
	  {
	      MPI_Recv(&local_best_pt[0], nvars, MPI_DOUBLE, diergasia, tag1, MPI_COMM_WORLD,  &status);
	      
	      MPI_Recv(&local_best_fx, 1, MPI_DOUBLE, diergasia, tag2, MPI_COMM_WORLD, &status);
	     
	      MPI_Recv(&local_best_trial, 1, MPI_INT, diergasia, tag3, MPI_COMM_WORLD,  &status);
		 
	      MPI_Recv(&local_best_jj, 1, MPI_INT, diergasia, tag4, MPI_COMM_WORLD,  &status);
